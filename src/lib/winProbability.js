import { HALF_SECONDS, balls, hasHalves, matchElapsedSeconds, score, wickets, } from "./matches";
...
function cricketInsight(match) {
    ...
    if (need <= 0)
        probB = 1;
    else if (ballsLeft === 0 || wktLeft === 0)
        probB = 0;
    else
        probB = sigmoid(0.9 * (currRR - reqRR) + 0.22 * (wktLeft - 5));
    return {
...
export function matchInsight(match) {
