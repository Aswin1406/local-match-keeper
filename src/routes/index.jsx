...
import { SPORT_META, initials, overs, scoreLine, } from "@/lib/matches";
export const Route = createFileRoute("/")({
    head: () => ({
        meta: [
            ...
            {
                name: "description",
                content: "Live, upcoming and completed local Cricket, Kabaddi and Football matches, scored by hand on your device.",
            },
    ...
    return (<div className="min-h-screen w-full bg-night text-ink">
      <div className="mx-auto w-full max-w-[880px] pb-10">
        ...
        <section className="px-5 pt-2">
          ...
          <Link to="/new" className="mt-5 w-full rounded-2xl bg-gradient-to-r from-gold to-lime text-night font-bold text-[15px] py-4 flex items-center justify-center gap-2">
            <span className="text-lg leading-none">＋</span> Create new match
          </Link>
        </section>

        <nav className="flex gap-2 px-5 mt-6 overflow-x-auto no-scrollbar">
          {["live", "upcoming", "completed"].map((t) => (<button key={t} onClick={() => setTab(t)} className={`shrink-0 rounded-full px-4 py-2.5 text-[13px] capitalize ${tab === t
        ...
        {featured ? (<section className="px-5 mt-4">
        ...
        <section className="px-5 mt-6">
          ...
          {!ready ? null : list.length === 0 ? (<div className="rounded-2xl border border-white/6 bg-panel p-6 text-center">
              <p className="text-[13px] text-mist">Nothing here yet.</p>
              <Link to="/new" className="mt-2 inline-block text-[12px] font-bold text-gold">
                Create a match
              </Link>
            </div>) : (<div className="space-y-2.5 sm:grid sm:grid-cols-2 sm:gap-2.5 sm:space-y-0">
...
function FeaturedMatch({ match }) {
    ...
    return (<div className="rounded-3xl bg-gradient-to-br from-panel2 to-panel border border-white/8 p-5 shadow-[0_20px_60px_-30px_#000]">
      ...
      <div className="mt-5 rounded-2xl bg-night/60 border border-white/5 p-3">
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          {last ? (<span className="text-[11px] text-mist">
              · {last.team === "a" ? match.teamA.name : match.teamB.name}
            </span>) : null}
        </div>
      </div>

      <Link to="/match/$id" params={{ id: match.id }} className="mt-4 block w-full rounded-2xl bg-teal text-night font-bold text-[15px] py-4 text-center">
        Open scorekeeper
      </Link>
    </div>);
}
function TeamBadge({ name, tone }) {
    ...
    return (<div className="flex-1 min-w-0 text-center">
      <div className={`size-12 rounded-full bg-gradient-to-br border grid place-items-center mx-auto ${cls}`}>
