import type { Sport } from "@/lib/matches";
import { STAT_COLS, sortStats, type PlayerStat } from "@/lib/playerStats";
      ...
      <table className="w-full text-[12px]">
        <thead>
          <tr className="text-[10px] uppercase tracking-[0.12em] text-mist">
            ...
            {cols.map((c) => (
              <th key={c.key} className="py-1.5 px-1.5 text-right font-semibold">
                {c.label}
              </th>
            ))}
          </tr>
        ...
        <tbody>
          {sortStats(stats, sport).map((s) => (
            <tr key={`${s.team}-${s.name}`} className="border-t border-white/6">
              ...
              {cols.map((c) => {
                ...
                const tone =
                  c.key === "yellow" && v ? "text-yellow-card" : c.key === "red" && v ? "text-red-card" : v ? "text-ink" : "text-mist";
                return (
                  <td key={c.key} className={`py-2 px-1.5 text-right tabular-nums font-bold ${tone}`}>
                    {v}
                  </td>
                );
              })}
